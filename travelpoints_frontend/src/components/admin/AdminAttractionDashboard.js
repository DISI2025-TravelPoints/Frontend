...
const AdminAttractionDashboard = () => {
  ...
  const handleLocationSelected = (location) => {
    ...
    form.setFieldsValue({ coordinates: location });
        ...
        prev.map((item) => {
          if (item.id === id) {
            return {
              ...
              latitude: latitude,
              longitude: longitude,
              audioFilePath: updatedFile
                ? S3_BUCKET_FOLDER + updatedFile.name
                : item.audioFilePath, // update the audio file path if a new file is uploaded
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: "entryFee",
      key: "entryFee",
      render: (text, record) => (record.isAddButton ? "" : `$${text}`), // add a dollar sign
    },
    {
      title: "Longitude",
      dataIndex: "longitude",
        ...
        <Form form={form} layout="vertical">
          ...
          >
            <Input
              type="number"
              onChange={(e) => {
                setEntryFee(parseFloat(e.target.value));
              }}
            />
          </Form.Item>
          <Form.Item
          ...
          <Form.Item
            ...
            label="Audio File"
            rules={[
              { required: true, message: "Please input the audio file!" },
            ]}
          >
...
export default AdminAttractionDashboard;
